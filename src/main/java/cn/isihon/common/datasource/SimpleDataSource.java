package cn.isihon.common.datasource;
...
import cn.isihon.util.DatabaseUtil;
...
public class SimpleDataSource implements DataSource, AutoCloseable {
    ...
    public SimpleDataSource(
        ...
        oracleDriver = StringUtils.equals(driverClassName, "oracle.jdbc.OracleDriver");
    ...
    public Connection getConnection() throws SQLException {
        try {
            ...
            if (activeNum.get() >= maxActive) {
                throw new RuntimeException(String.format("数据库连接数超过上限%d，url=%s", maxActive, url));
            }
        ...
        try {
            simpleConnection =
                    new SimpleConnection(
                            DatabaseUtil.getConnection(driverClassName, url, username, password), oracleDriver);
